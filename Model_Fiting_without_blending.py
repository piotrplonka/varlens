...
def chi2_fun(params, time, flux, flux_error):
    t_0, u_0, t_E, baseline_source = params
    # Sprawdzanie ograniczeń na u_0
    if u_0 < 0 or u_0 > 1 or t_E < 0 or baseline_source < 0 or t_E > 1000: #or (baseline_blending+baseline_source) > 1.1*baseline_guess_flux:
        return np.inf  

    u = np.sqrt(u_0**2 + ((time - t_0) / t_E)**2)
    A = ((u**2 + 2) / (u * np.sqrt(u**2 + 4)))
    flux_calculated = baseline_source * A 
...
fixed_source = np.median(flux) 


# Iteracja tylko po u_0 i t_E
initial_guesses = [
    (fixed_t_0, u, t, fixed_source) 
    for u in guess_u_0 
    for t in guess_t_E
]
...
fit_t_0, fit_u_0, fit_t_E, fit_source  = best_params
print(f"Najlepsze parametry: t_0 = {fit_t_0}, u_0 = {fit_u_0}, t_E = {fit_t_E}, Source: = {fit_source}")
